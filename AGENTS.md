...
- Files are stored as logical absolute paths and belong to explicit roots; symbols and calls are stored per content (inode + size + mtime + language), so a hardlinked file is parsed once no matter how many roots link it. Freshness is a stat comparison — unchanged files are never read or hashed.
