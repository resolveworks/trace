...
- The daemon owns one persistent SQLite database and all directory watchers: one non-recursive `fs.watch` per included logical directory, never one per file. The schema is never migrated: on change, delete the database and the daemon rebuilds it from the roots.
- `node_modules` and `.venv` under a root are dependency environments: indexed despite `.gitignore`, stored at their logical (symlink-unresolved) paths, and excluded from queries whose scope is not inside them. Physical `.pnpm` store directories are never indexed or watched; packages are reached through logical directory symlinks. File symlinks are not indexed because their target changes cannot be observed by the logical parent watcher. The installed environment is authoritative, not the cache.
